...
export const UsersReducer = (state: IStatusizedUsers, action: IUsersAction): IStatusizedUsers => {
    switch (action.type) {
        case "not started": return { ...state, loading: "not started" };
        case "started": return { ...state, loading: "started" };
        case "CompleteUsers": return { ...state, users: action.payload, loading: "completed" };
    }
